...
void APIKey::fromJson(web::json::value& val)
{
    ...
    }
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("permissions")].as_array() )
        {
            if(item.is_null())
            {
                m_Permissions.push_back( std::shared_ptr<Xany>(nullptr) );
            }
            else
            {
                std::shared_ptr<Xany> newItem(new Xany());
...
std::vector<std::shared_ptr<Xany>>& APIKey::getPermissions()
{
    return m_Permissions;
}

void APIKey::setPermissions(std::vector<std::shared_ptr<Xany>> value)
{
    m_Permissions = value;
